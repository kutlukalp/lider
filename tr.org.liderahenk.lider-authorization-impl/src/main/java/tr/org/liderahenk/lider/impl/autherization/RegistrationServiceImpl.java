...
import tr.org.liderahenk.lider.core.api.autherization.IRegistrationService;
...
public class RegistrationServiceImpl implements IRegistrationService {
	...
	@Override
	public IRegistrationInfo register(String identifier, String member,
		...
		String entryDN = entryAttributes.get("DN")[0];//(String) idMatcher.match(identifier);
		String[] attrs = {"uid"};
		...
		if(!entry.isEmpty()){
			if(!useExistingEntry){
				return new RegistrationInfoImpl(RegistrationStatus.ALREADY_EXISTS,
					null,
					null, null, null,
					entry.get(0).getDistinguishedName() + " already exists! Set useExistingEntry flag to true to retrieve & use that information");
			}
			else{
				...
				return new RegistrationInfoImpl(RegistrationStatus.PASSWORD_UPDATED,
						...
						idMatcher.getXmppServer(),
						idMatcher.getXmppDomain(),
						//"http://localhost:8181/",
						entry.get(0).getDistinguishedName() + " already exists! Updated its password with the one you submitted and returning existing entry's attributes");
			}
		}
		else /*if( entry == null )*/{
			...
			try {
				...
				LdapEntry newEntry = ldapService.getEntry(entryDN, new String[]{idMatcher.getAgentJidAttribute(), "userPassword"});
				return new RegistrationInfoImpl(RegistrationStatus.REGISTERED,
						newEntry.getAttributes().get(idMatcher.getAgentJidAttribute()),
						entryDN,
						idMatcher.getXmppServer(),
						idMatcher.getXmppDomain(),
						entryDN + " created successfully!");
				
			} catch (final LdapException e) {
				return new RegistrationInfoImpl( 
